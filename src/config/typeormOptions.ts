...
import { entities } from '../werkstatt/entity/entities.js';
import { Werkstatt } from '../werkstatt/entity/werkstatt.entity.js';
...
const database =
    (db?.name as string | undefined) ?? Werkstatt.name.toLowerCase();

const host = (db?.host as string | undefined) ?? 'localhost';
const username =
    (db?.username as string | undefined) ?? Werkstatt.name.toLowerCase();
